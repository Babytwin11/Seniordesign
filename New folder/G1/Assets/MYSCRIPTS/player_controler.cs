...
namespace S3
{
    public class player_controler : NetworkBehaviour
    {
        ...
        public GameObject sphereprefabe;
        public Transform spherespawne;
        public int TeamIde;
       ...
       public void CmdFire()
        {
            ...
            { //Create the bullet from the prefab
                ...
                Destroy(Sphere, 1);
              //  TeamIde = Sphere.GetComponent<Team>().TeamId;
             //   Debug.Log(TeamIde);
            }
            else
            {
                //Create the bullet from the prefab
                GameObject Spheree = (GameObject)Instantiate(sphereprefabe, spherespawne.position, spherespawne.rotation);
                Debug.Log("hi");
                //Add velocity to the bullet
                // Sphere.GetComponent<Rigidbody>().velocity = Sphere.transform.forward * 2.0f;

                //Spawn the bullet on the Clients
                NetworkServer.Spawn(Spheree);

                //Destroy the bullet after 2 seconds
                Destroy(Spheree, 1);
               // TeamIde = Spheree.GetComponent<Team>().TeamId;
               // Debug.Log(TeamIde);
